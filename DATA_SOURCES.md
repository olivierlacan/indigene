...
| **European Lepidoptera–Plant matrix** (Gaytán et al. 2026, *Ecology & Evolution* [doi:10.1002/ece3.73004](https://doi.org/10.1002/ece3.73004)) | The European `hostLepCount` — Lepidoptera host-species counts per plant genus (the Tallamy/NWF equivalent for Europe) | *Canonical source, integration pending* — the France rows currently carry flagged genus-level estimates until the recompute lands | **CC-BY 4.0**, machine-readable | ✅ **Identified & preferred.** 5,152 Lepidoptera × 3,275 plants, all Europe, species-level → aggregate to genus for counts on the same scale as the US regions. Cleaner terms than NWF. A `scripts/build-host-counts.mjs` step will replace the estimates; keep the shared `HOST_ANCHOR` so US scores don't move. See `docs/france-localization-plan.md` §4.3. |
| **DBIF v2** (CEH/BRC *Database of Insects and their Food Plants*) | GB cross-check for the European host counts — ships a per-plant "insect species richness" file | Referenced / cross-check for the Atlantic France list | **Open Government Licence** ("Contains data supplied by NERC") | ✅ Safe with attribution. Great Britain only, but the Atlantic-France flora overlaps almost entirely. |
| **BRC DBIF / Southwood foliage-insect rankings** | Genus-level host-insect context behind the *interim* France estimates | Basis for each France row's `hostLepCount` **until** the matrix recompute | Facts referenced, not prose | ⚠️ **Interim estimates, flagged.** France host counts are honest genus-level estimates anchored on these rankings, each row's `confidence` set accordingly, pending the Gaytán-matrix recompute above. |
