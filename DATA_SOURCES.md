...
| **RESOLVE Ecoregions 2017** (Dinerstein et al., *BioScience* 2017) | Ecoregion label + region selection south of the equator | Live point query to Esri's hosted copy (`services.arcgis.com/.../Resolve_Ecoregions/FeatureServer/0`), best-effort, asked only when `lat < 0` | **CC-BY 4.0** | 🟡 **Wired, unconfirmed.** Same job as EPA/EEA for the southern regions (`docs/southern-hemisphere-plan.md`); `code` is the numeric `ECO_ID`. The build sandbox's egress refuses the host, so the layer id, field names and CORS are the published dataset's until `npm run probe:resolve` answers from an unblocked machine (`data/sources/resolve-ecoregions/probe.json`). No shipped region declares it yet, so nothing selects on it. Attribution: "RESOLVE Ecoregions 2017 (CC BY 4.0)". |
...
| **RESOLVE Ecoregions 2017** (successor to WWF's Terrestrial Ecoregions of the World) | Global | CC BY 4.0 | The southern-hemisphere equivalent — wired into `site.ts` for points south of the equator and into the map builder, **pending confirmation** of the hosted layer (`npm run probe:resolve`; see `data/sources/resolve-ecoregions/`). |
