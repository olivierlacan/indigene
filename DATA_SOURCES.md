...
| **BigDataCloud reverse geocoding** (`api.bigdatacloud.net/data/reverse-geocode-client`) | Coordinates → nearest town name, shown instead of raw lat/lon | Free client-side REST API, no key, CORS-enabled, explicitly offered for browser use | Free tier for client-side use | ✅ Display-only nicety: no verdict, lookup, or stored datum depends on it, and every caller falls back to coordinates when it fails. |
