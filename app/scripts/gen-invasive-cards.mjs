...
<div class="wash"></div>
<header>${mark}<span class="wordmark">Indigene</span><span class="badge">Worst invasives</span></header>
<div class="mid">
  <div class="names">
    <h1>${esc(name)}</h1>
  ...
  const invasives = INVASIVES.filter((i) => ids.has(i.id)).map((inv) => {
    ...
    return {
      ...
      facts: [
        ...
        { icon: "flag", value: `#${best}`, label: "highest place on a list" },
      ],
    };
  });
  const indexCard = {
