...
// Name resolution, synonymy and the homonym guard live in `_wcvp.mjs`, shared
// with `check-native.mjs` — see that file for why asking WCVP has a trap in it.

console.log("\n  asking WCVP (World Checklist of Vascular Plants, via GBIF) — the accepted name and its native range");
let wcvpAsked = 0;
for (const c of shortlist) {
  try {
    const usage = await wcvpAccepted(c.latin);
    await sleep(350);
    if (usage) {
