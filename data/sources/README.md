...
| [`hosts/`](hosts/) | HOSTS, the NHM's world database of Lepidoptera host plants. `records/<area>.csv` archives every record for Australia, southern Africa, the Río de la Plata and New Zealand; `genus-counts.json` counts moth species per host genus. The 48 MB full download is git-ignored | CC0 (NHM) | `cd app && npm run hosts:fetch` |
...
| [`wildflower-center/`](wildflower-center/) | `<region>.json` — the same verdict from a second society list, added because Audubon's western coverage is too thin to measure against. Per-state Recommended Species; **no flora tier**, so it can show a gap but never settle native status. Raw harvest git-ignored | audit read; selection is the Center's | `cd app && npm run harvest:wildflower` then `npm run societies` |
| [`audubon-plants-for-birds/`](audubon-plants-for-birds/) | `<region>.json` — what a native plant society's own list says about the rows **we** ship: did their curation pick it, does their flora list it here at all, and which genera they carry that we never have. The raw per-ZIP harvest in `raw/` is **git-ignored** (their plant data is BONAP-derived, which DATA_SOURCES.md forbids embedding) | audit read; plant data BONAP, curation Audubon | `cd app && npm run harvest:audubon` then `npm run societies` |
